...
const myDayManagerSource = readFileSync(
    new URL("../src/kernel/my-day-manager.ts", import.meta.url),
    "utf8",
);
...
test("任务完成会写入我的一天实例，重复任务内部重开后清除完成状态", () => {
    ...
    assert.match(taskServiceSource, /!advanced\.ended && advanced\.state\.status === "active"[\s\S]*myDayManager\.clearTaskCompleted\(blockId\)/);
});
