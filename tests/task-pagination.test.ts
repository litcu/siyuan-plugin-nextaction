...
test("completed sorting falls back to updated and keeps missing due dates last", () => {
    ...
    assert.deepEqual(
        fallback.map((entry) => entry.blockId),
        ["new", "old"],
    );

    const due = sortCompletedTasks([task("missing", { due: "" }), task("dated", { due: "2026-04-01" })], "due", false);
    assert.deepEqual(
        due.map((entry) => entry.blockId),
        ["dated", "missing"],
    );
});

test("pagination clamps page and limits page size", () => {
    const tasks = Array.from({ length: 121 }, (_, index) =>
...
test("pagination ignores non-completed entries and reports hasMore", () => {
    const result = paginateCompletedTasks([task("done-1"), task("todo", { status: "todo" }), task("done-2")], {
        page: 1,
        pageSize: 1,
    });
