...
test("AI 提案解析支持 JSON 和 markdown fenced JSON", () => {
    assert.deepEqual(parseAiJson('{"feature":"review","summary":"ok","review":{"summary":"ok","groups":[],"actions":[]}}'), {
...
test("AI 空任务提案可以表示没有识别到可执行任务", () => {
    const result = validateAiProposal({ feature: "extractTasks", summary: "没有可执行任务", tasks: [], warnings: ["无任务"] });
...
test("AI My Day 提案只接受合法任务 block ID", () => {
    const result = validateAiProposal({ feature: "planMyDay", summary: "选择", myDay: [{ blockId: "bad", reason: "x" }] });
...
test("AI 回顾提案只读且必须包含报告", () => {
    ...
    const valid = validateAiProposal({ feature: "review", summary: "x", review: { summary: "x", groups: [{ key: "overdue", title: "逾期", summary: "需要处理" }], actions: [] } });
...
test("AI 回顾会补齐模型遗漏的源分组任务", () => {
    const result = completeAiReviewGroups({
        ...
        review: {
            summary: "回顾",
            groups: [{ key: "reviewDue", title: "待回顾", summary: "模型摘要", blockIds: ["20260802120000-zzzzzzz"] }],
            actions: [],
        },
    }, { reviewDue: ["20260802120000-abcdefg"] }, { reviewDue: "待回顾" });
    ...
    const service = new AiProposalService(
        {
            ...
            addTaskToMyDay: async (id: string) => { calls.push(id); return { schema: 1, dayKey: "2026-08-02", updatedAt: 2, tasks: [{ blockId: id, addedAt: 1, scheduleStart: null, scheduleEnd: null, order: 0 }] }; },
        } as any,
        async () => { throw new Error("not expected"); },
        async () => { throw new Error("not expected"); },
    );
    const result = await service.apply({ feature: "planMyDay", summary: "选择", myDay: [{ blockId: task.blockId, reason: "due" }] });
    assert.deepEqual(calls, [task.blockId]);
    assert.equal(result.myDay?.tasks[0].scheduleStart, null);
});

test("AI 提案服务拒绝回顾写入", async () => {
    const service = new AiProposalService({} as any, async () => { throw new Error(); }, async () => { throw new Error(); });
    await assert.rejects(() => service.apply({ feature: "review", summary: "x", review: { summary: "x", groups: [], actions: [] } }), /read-only/);
});
