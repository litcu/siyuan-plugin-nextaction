...
function definitions(): Record<McpToolName, ToolDefinition> {
    return Object.fromEntries(ALL_NAMES.map(name => [name, {
        title: name,
...
function fakeExecutor(catalog: McpToolCatalog) {
    ...
    const executor = {
        updateSettings(settings: unknown) { settingsSeen.push(settings); },
        getCatalog() { return catalog; },
        createHandler(name: McpToolName) { return catalog.get(name).handler; },
...
function fakeSiyuan(agent?: {
    ...
    return {
        agent,
        logger: {
            info: async (message: string) => { messages.push(message); },
            warn: async (message: string) => { messages.push(message); },
            error: async (message: string) => { messages.push(message); },
        },
...
test("MCP catalog 唯一覆盖 8 个只读和 6 个写入工具", () => {
    ...
    assert.deepEqual(entries.map(entry => entry.name), ALL_NAMES);
