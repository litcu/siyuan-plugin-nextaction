...
const pluginEntry = readFileSync(new URL("../src/index.ts", import.meta.url), "utf8");
...
test("插件启动和内核重连时先加载自定义字段设置再加载任务", () => {
    ...
    assert.match(pluginEntry, /kernel-plugin-state-change[\s\S]*?await this\.loadTaskStoreState\(\)/);
