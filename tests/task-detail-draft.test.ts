...
test("立即保存仅在存在合法待同步草稿时启用", () => {
    assert.equal(canSaveTaskDetailNow({ dirty: false, saving: false, hasValidationError: false, operationBusy: false }), false);
    assert.equal(canSaveTaskDetailNow({ dirty: true, saving: false, hasValidationError: false, operationBusy: false }), true);
    assert.equal(canSaveTaskDetailNow({ dirty: true, saving: true, hasValidationError: false, operationBusy: false }), false);
    assert.equal(canSaveTaskDetailNow({ dirty: true, saving: false, hasValidationError: true, operationBusy: false }), false);
});
