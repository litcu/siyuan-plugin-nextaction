...
test("AI extraction slash callback uses the persisted cleanup helper", () => {
    assert.match(
        source,
        /id: "aiExtractTasks",[\s\S]*?this\.clearSlashCommand\(protyle, nodeElement\);[\s\S]*?runAiExtractTasks\(\[blockId\]\)/,
    );
});
...
test("slash menu supports semantic and legacy project triggers", () => {
    assert.match(
        source,
        /filter:\s*\[this\.plugin\.i18n\.convertToProject,\s*"convert to project",\s*"nproject",\s*"zxm"\]/,
    );
});
...
test("slash menu supports semantic and legacy subtree task triggers", () => {
    assert.match(
        source,
        /filter:\s*\[\s*this\.plugin\.i18n\.convertToTaskWithChildren,\s*"convert to task with children",\s*"ntaskchildren",\s*"zrwz",?\s*\]/,
    );
});
