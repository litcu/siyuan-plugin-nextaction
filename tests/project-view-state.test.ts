...
test("完成项开关、任务筛选和三类项目筛选可以组合", () => {
    ...
    assert.deepEqual(hiddenDone.detailTasks.map(item => item.blockId), ["a"]);
    const shownDone = buildProjectViewModel(projects, [], state({ activeProjectId: "p1", showCompleted: true }));
    assert.deepEqual(shownDone.detailTasks.map(item => item.blockId), ["a", "done"]);
    const searched = buildProjectViewModel(projects, [], state({
        showCompleted: true,
        filterState: { ...DEFAULT_FILTER_STATE, searchText: "finished" },
    }));
    assert.deepEqual(searched.visibleSummaries.map(item => item.project.blockId), ["p1"]);
    const available = buildProjectViewModel(projects, [], state({ actionFilter: "available" }));
    assert.deepEqual(available.visibleSummaries.map(item => item.project.blockId), ["p1"]);
    const missing = buildProjectViewModel(projects, [], state({ actionFilter: "missing" }));
    assert.deepEqual(missing.visibleSummaries.map(item => item.project.blockId), []);
});

test("选中任务自动定位项目且 override 立即进入视图模型", () => {
    const override = { ...projects[1], title: "edited before broadcast", status: "doing" };
    const model = buildProjectViewModel(projects, [], state({
        activeProjectId: "p2",
        selectedTaskId: "a",
...
test("折叠状态和甘特排序进入树模型但不丢失完整任务集合", () => {
    ...
    assert.deepEqual(model.projectTreeModel?.rows.map(row => row.task.blockId), ["p", "parent"]);
    assert.deepEqual(model.projectTreeModel?.includedTasks.map(item => item.blockId), ["p", "parent", "child"]);
});

test("看板移动先更新状态再重排并向上抛出失败", async () => {
    const calls: string[] = [];
    await executeProjectBoardMove({ task: projects[1], status: "doing", afterId: "after" }, "p1", {
        updateTask: async (_task, attrs) => { calls.push(`update:${attrs["na-status"]}`); },
        reorderTask: async (blockId, parentId, afterId) => { calls.push(`reorder:${blockId}:${parentId}:${afterId}`); },
    });
    assert.deepEqual(calls, ["update:doing", "reorder:a:p1:after"]);

    await assert.rejects(() => executeProjectBoardMove({ task: projects[1], status: "doing" }, "p1", {
        updateTask: async () => { throw new Error("write failed"); },
        reorderTask: async () => { calls.push("unexpected reorder"); },
    }), /write failed/);
