...
export class TaskCustomFieldService {
    ...
    getCustomFieldDiagnostics(): {
        ...
        return {
            fields: this.runtime.getSettings().customFields.map((field) => ({
                fieldId: field.id,
                key: field.key,
                status: field.status,
                count: counts.get(field.key) || 0,
            })),
            orphans: [...orphanSamples.entries()].map(([key, sampleBlockIds]) => ({
                key,
                count: this.cacheManager
                    .getAll()
                    .filter((entry) => Object.prototype.hasOwnProperty.call(entry.customFields || {}, key)).length,
                sampleBlockIds,
            })),
        };
    }
    ...
    private async clearCustomFieldValues(fieldKey: string): Promise<{ cleared: number; failedBlockIds: string[] }> {
        ...
        const targets = this.cacheManager
            .getAll()
