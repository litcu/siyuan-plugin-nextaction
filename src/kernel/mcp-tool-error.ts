...
export function normalizeMcpToolError(error: unknown): Error {
    ...
    if (code === RPC_ERROR_NOT_READY) return new McpToolError("NOT_READY", String(details.message || "Kernel is not ready"));
    if (code === RPC_ERROR_TASK_NOT_FOUND) return new McpToolError("TASK_NOT_FOUND", String(details.message || "Task not found"));
    if (code === RPC_ERROR_INVALID_PARAMS) return new McpToolError("INVALID_INPUT", String(details.message || "Invalid input"));
    if (code === RPC_ERROR_PROJECT_REQUIRES_DOCUMENT) {
        return new McpToolError("INVALID_INPUT", String(details.message || "Only document blocks can be converted to projects"));
    }
