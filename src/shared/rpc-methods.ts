...
import { CREATE_TASK_DESTINATION_TYPES, CREATE_TASK_FORMATS, type CreateTaskDestination, type CreateTaskInput, type CreateTaskResult } from "./task-creation";
import type {
    CompletedTasksPage,
    MyDayState,
    ReviewData,
...
function createTaskParams(value: unknown): CreateTaskInput {
    ...
    if (input.destination !== undefined) {
        ...
        if (rawDestination.format !== undefined && !(CREATE_TASK_FORMATS as readonly unknown[]).includes(rawDestination.format)) {
...
export const RPC_CONTRACT = {
    echo: defineRpc<{ params: unknown[] }, unknown[]>((value) => {
        const input = paramsRecord(value);
        if (input.params !== undefined && !Array.isArray(input.params)) throw new RpcContractError("params must be an array");
    ...
    convertToTask: defineRpc<{ blockId: string; cleanTitle?: string; taskType?: string }, TaskCacheEntry>((value) => {
        ...
        if (taskType !== undefined && taskType !== "1" && taskType !== "2") throw new RpcContractError("taskType must be 1 or 2");
        return { blockId: requiredBlockId(input.blockId), cleanTitle: optionalString(input.cleanTitle, "cleanTitle"), taskType };
    }),
    convertToTaskWithChildren: defineRpc<{ blockId: string; cleanTitle?: string; taskType?: string }, { converted: number; skipped: number }>((value) => {
        const input = paramsRecord(value);
        const taskType = optionalString(input.taskType, "taskType");
        if (taskType !== undefined && taskType !== "1" && taskType !== "2") throw new RpcContractError("taskType must be 1 or 2");
        return { blockId: requiredBlockId(input.blockId), cleanTitle: optionalString(input.cleanTitle, "cleanTitle"), taskType };
    ...
    setRepeatRule: defineRpc<{ blockId: string; rule: RepeatRuleV2 }, TaskCacheEntry>((value) => {
        const input = paramsRecord(value);
        return { blockId: requiredBlockId(input.blockId), rule: requiredObject(input.rule, "rule") as unknown as RepeatRuleV2 };
    }),
    ...
    getCompletedTasksPage: defineRpc<CompletedTasksPageOptions, CompletedTasksPage>((value) => {
        ...
        if (input.sortBy !== undefined && typeof input.sortBy !== "string") throw new RpcContractError("sortBy must be a string");
        if (input.sortAsc !== undefined && typeof input.sortAsc !== "boolean") throw new RpcContractError("sortAsc must be boolean");
    ...
    reorderTask: defineRpc<{ blockId: string; parentId?: string | null; afterId?: string | null }, TaskCacheEntry>((value) => {
        const input = paramsRecord(value);
        return { blockId: requiredBlockId(input.blockId), parentId: optionalBlockId(input.parentId, "parentId"), afterId: optionalBlockId(input.afterId, "afterId") };
    }),
    getStatistics: defineRpc<{ period?: "week" | "month" }, StatisticsResult>((value) => {
        const input = paramsRecord(value);
        if (input.period !== undefined && input.period !== "week" && input.period !== "month") throw new RpcContractError("period must be week or month");
    ...
    validateAiProposal: defineRpc<{ proposal: AiProposal }, { proposal: AiProposal; errors: string[] }>(rawProposalParams),
    ...
    listMcpTargetDocuments: defineRpc<{ notebookId: string; path?: string }, { notebookId: string; path: string; items: RpcMcpDocumentListItem[] }>((value) => {
        const input = paramsRecord(value);
    ...
    setMyDaySchedule: defineRpc<{ blockId: string; start: number | null; end: number | null }, MyDayState>((value) => {
        ...
        const end = input.end;
        if (start !== null && (typeof start !== "number" || !Number.isFinite(start))) throw new RpcContractError("start must be a number or null");
        if (end !== null && (typeof end !== "number" || !Number.isFinite(end))) throw new RpcContractError("end must be a number or null");
    ...
    markTaskReviewed: defineRpc<{ blockIds: string[] }, TaskCacheEntry[]>((value) => {
        const input = paramsRecord(value);
        if (!Array.isArray(input.blockIds) || input.blockIds.length === 0) throw new RpcContractError("blockIds must be a non-empty array");
...
export type RpcReturn<M extends RpcMethodName> = RpcContract[M] extends RpcDefinition<unknown, infer Result> ? Result : never;
