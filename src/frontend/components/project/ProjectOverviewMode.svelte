...
<div class="na-project-overview">
    <section class="na-project-section na-project-section--risks">
        <div class="na-project-section__heading"><h3>{i18n?.projectRisks || "Risks"}</h3><span>{summary.risks.length}</span></div>
    ...
    <section class="na-project-section">
        <div class="na-project-section__heading"><h3>{i18n?.projectNextActions || "Next actions"}</h3><span>{summary.nextActions.length}</span></div>
    ...
    <section class="na-project-section">
        ...
        <dl class="na-project-facts">
            <div><dt>{i18n?.status || "Status"}</dt><dd>{statusLabel(summary.project.status)}</dd></div>
            <div><dt>{i18n?.dueDate || "Due"}</dt><dd>{summary.project.due || i18n?.projectNoDue || "No date"}</dd></div>
            <div><dt>{i18n?.projectWaiting || "Waiting"}</dt><dd>{summary.waitingTasks.length}</dd></div>
            <div><dt>{i18n?.blocked || "Blocked"}</dt><dd>{summary.blockedTasks.length}</dd></div>
        </dl>
    </section>
</div>
...
<style lang="scss">
    ...
    .na-project-risk { display: flex; align-items: flex-start; gap: 8px; width: 100%; padding: 7px 0; border: 0; color: var(--na-text-primary); background: transparent; text-align: left; cursor: pointer; }
    .na-project-risk:hover { color: var(--na-accent); }
    .na-project-risk > span:last-child { display: flex; flex-direction: column; min-width: 0; gap: 2px; }
    .na-project-risk strong { font-size: var(--na-font-size-sm); font-weight: 650; }
    .na-project-risk small { color: var(--na-text-primary); font-size: var(--na-font-size-sm); }
    ...
    .na-project-risk__marker--high { background: var(--na-color-error); }
    .na-project-risk__marker--medium { background: var(--na-color-warning); }
    .na-project-risk__marker--low { background: var(--na-color-info); }
    ...
    .na-project-facts dt { color: var(--na-text-secondary); font-size: var(--na-font-size-xs); }
    .na-project-facts dd { margin: 2px 0 0; overflow: hidden; color: var(--na-text-primary); font-size: var(--na-font-size-sm); text-overflow: ellipsis; white-space: nowrap; }
    @container nextaction-app (max-width: 880px) { .na-project-overview { grid-template-columns: 1fr; } }
</style>
