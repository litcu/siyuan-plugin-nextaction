        ...
        <section class="na-project-board__column" role="list" class:drop-active={dropStatus === status} on:dragover|preventDefault={() => dropStatus = status} on:dragleave={() => dropStatus = ""} on:drop|preventDefault={() => handleDrop(status)}>
            ...
            <div class="na-project-board__cards">
                {#each statusTasks as task (task.blockId)}
                    <div class="na-project-board__card" role="listitem" draggable={!busy} on:dragstart={(event) => handleDragStart(task, event)} on:dragend={resetDrag} on:dragover|preventDefault={() => dropStatus = status} on:drop|preventDefault|stopPropagation={() => handleDrop(status, task.blockId)}>
                ...
                {/each}
                {#if statusTasks.length === 0}<p class="na-project-board__empty">{i18n?.projectDropHere || "Drop tasks here"}</p>{/if}
            </div>
        </section>
    {/each}
</div>
...
<style lang="scss">
    ...
    .na-project-board__column > header { display: flex; justify-content: space-between; padding: 9px 10px; border-bottom: 1px solid var(--na-color-divider); color: var(--na-text-secondary); font-size: var(--na-font-size-xs); font-weight: 700; text-transform: uppercase; }
    .na-project-board__cards { display: flex; flex-direction: column; gap: 5px; padding: 6px; min-height: 220px; }
    .na-project-board__card { cursor: grab; }
    .na-project-board__card:active { cursor: grabbing; }
    .na-project-board__empty { margin: 14px 6px; color: var(--na-text-secondary); font-size: var(--na-font-size-xs); text-align: center; }
    @container nextaction-app (max-width: 780px) { .na-project-board { min-width: 760px; } }
</style>
