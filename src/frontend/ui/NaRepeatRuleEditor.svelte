...
>
    {#if error}<NaInlineNotice slot="notice" message={error} tone="error" />{:else if validationError}<NaInlineNotice slot="notice" message={validationError} tone="warning" />{/if}
        ...
        <NaPropertySection title={i18n?.repeatWeekdays || "Weekdays"}>
            <div class="na-repeat-rule-editor__weekdays">
                {#each weekdayLabels as label, index}
                    <button type="button" class:active={weekdays.includes(weekdayAt(index))} aria-pressed={weekdays.includes(weekdayAt(index))} on:click={() => toggleWeekday(weekdayAt(index))}>{label.slice(0, 2)}</button>
                {/each}
            </div>
        </NaPropertySection>
    {/if}
    ...
    {#if frequency === "month"}
        <NaPropertySection title={i18n?.repeatMonthlyPattern || "Monthly pattern"}>
            ...
            {#if monthlyType === "dayOfMonth"}
                <NaPropertyRow label={i18n?.repeatMonthDay || "Day"}><input class="b3-text-field" type="number" min="1" max="31" bind:value={monthDay} /></NaPropertyRow>
                <NaPropertyRow label={i18n?.repeatOverflow || "Missing day"}>
                    <select class="b3-select" bind:value={overflow}><option value="lastDay">{i18n?.repeatOverflowLastDay || "Use last day"}</option><option value="skip">{i18n?.repeatOverflowSkip || "Skip month"}</option></select>
                </NaPropertyRow>
            {:else if monthlyType === "nthWeekday"}
                <NaPropertyRow label={i18n?.repeatNthWeekday || "Ordinal"}>
                    <select class="b3-select" bind:value={monthlyNth}><option value="1">{i18n?.ordinalFirst || "First"}</option><option value="2">{i18n?.ordinalSecond || "Second"}</option><option value="3">{i18n?.ordinalThird || "Third"}</option><option value="4">{i18n?.ordinalFourth || "Fourth"}</option><option value="-1">{i18n?.ordinalLast || "Last"}</option></select>
                </NaPropertyRow>
                <NaPropertyRow label={i18n?.repeatWeekdays || "Weekday"}>
                    <select class="b3-select" bind:value={monthlyWeekday}>{#each weekdayLabels as label, index}<option value={index + 1}>{label}</option>{/each}</select>
                </NaPropertyRow>
            {/if}
        </NaPropertySection>
    {/if}
        ...
        {#if previews.length > 0}
            <ol class="na-repeat-rule-editor__preview">
                {#each previews as item, index}
                    <li><span>{index + 1}</span><strong>{item.start || item.due}</strong>{#if item.start && item.due && item.start !== item.due}<small>{item.due}</small>{/if}</li>
                {/each}
            </ol>
        {:else}
...
<style lang="scss">
    ...
    .na-repeat-rule-editor__weekdays button:hover { border-color: var(--b3-theme-primary-light); color: var(--b3-theme-on-surface); }
    .na-repeat-rule-editor__presets button.active,
    .na-repeat-rule-editor__weekdays button.active { border-color: var(--b3-theme-primary); color: var(--b3-theme-primary); background: var(--b3-theme-primary-lightest); }
    .na-repeat-rule-editor__weekdays { display: grid; grid-template-columns: repeat(7, minmax(0, 1fr)); gap: 5px; padding: 10px 0; }
    :global(.na-repeat-rule-editor .na-segment-control) { width: 100%; }
    .na-repeat-rule-editor__preview { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 6px; margin: 0; padding: 10px 0; list-style: none; }
    .na-repeat-rule-editor__preview li { display: grid; grid-template-columns: 20px minmax(0, 1fr); align-items: center; gap: 1px 7px; min-width: 0; padding: 7px 8px; border: 1px solid var(--b3-border-color); border-radius: var(--b3-border-radius); background: var(--b3-theme-background); }
    .na-repeat-rule-editor__preview li > span { grid-row: 1 / 3; color: var(--b3-theme-primary); font-size: 10px; font-weight: 600; }
    .na-repeat-rule-editor__preview strong { overflow: hidden; color: var(--b3-theme-on-surface); font-size: 11px; text-overflow: ellipsis; white-space: nowrap; }
    .na-repeat-rule-editor__preview small { color: var(--b3-theme-on-surface-light); font-size: 10px; }
    .na-repeat-rule-editor__empty { padding: 18px 0; color: var(--b3-theme-on-surface-light); font-size: var(--na-font-size-md); text-align: center; }
    @media (max-width: 520px) {
        .na-repeat-rule-editor__presets { grid-template-columns: repeat(2, minmax(0, 1fr)); }
        .na-repeat-rule-editor__preview { grid-template-columns: 1fr; }
    }
</style>
