...
function deriveProjectReminders(allTasks: TaskCacheEntry[]): TaskCacheEntry[] {
    ...
    return allTasks.filter(entry => entry.taskType === "2"
        && entry.status !== "done"
...
export function reduceTaskChanges(
    ...
    const allTasks = [...byId.values()].map(task => task.childIds.some(childId => deletedIds.has(childId))
...
function isTaskEntry(value: unknown): value is TaskCacheEntry {
    ...
    const stringFields: Array<keyof TaskCacheEntry> = [
        "blockId", "parentId", "status", "priority", "due", "start", "context", "taskType", "title",
        "depends", "depMode", "repeat", "repeatState", "completed", "note", "created", "tags",
        "blockedReason", "reviewDate", "reminder",
    ];
    ...
    return stringFields.every(field => typeof task[field] === "string")
...
export function isTaskChangeSetV2(value: unknown): value is TaskChangeSetV2 {
    ...
    if (changeSet.type !== "delta"
        ...
        || !changeSet.deletedBlockIds.every(blockId => typeof blockId === "string")) return false;
    const deletedIds = new Set(changeSet.deletedBlockIds);
    return deletedIds.size === changeSet.deletedBlockIds.length
